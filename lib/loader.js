...
module.exports = function (source) {
  ...
  function getResourceQuery(ctx) {
    return (0, loaderUtils.parseQuery)(ctx.resourceQuery || "?");
  }

  var data = _.defaults(options, { variable: 'data' }, getResourceQuery(this));
  ...
  return 'var _ = require(' + loaderUtils.stringifyRequest(this, require.resolve('lodash')) + ');' +
    ...
    Object.keys(data).filter(k => !!k).map(function (queryVariableName) {
      return 'var ' + queryVariableName + ' = ' + JSON.stringify(data[queryVariableName]);
    }).join(';') + ';' +
